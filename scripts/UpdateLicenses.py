...
def get_license_data():
    if "upslide-automation" not in os.getcwd(): os.chdir('upslide-automation')

    with open(r'config/upslide-credentials.json') as jsonFile:
        content = json.load(jsonFile)
        USERNAME = content['USER']
        PASSWORD = content['PASSWORD']
        DOWNLOAD = os.getcwd() + r"\data\report"
        ARCHIVE = os.getcwd() + r"\data\archive"

    ## Archive license data
    downloadTS = datetime.now().strftime("%Y_%m_%d")
    if "licenses.csv" in os.listdir(DOWNLOAD):
        shutil.copy(f"{DOWNLOAD}/licenses.csv", f"{ARCHIVE}/licenses_{downloadTS}.csv")
        os.remove(f"{DOWNLOAD}/licenses.csv")
    ...
    if "users.csv" in os.listdir(DOWNLOAD):
        downloadTS = datetime.now().strftime("%Y_%m_%d")
        if f"{DOWNLOAD}/licenses.csv" not in os.listdir(DOWNLOAD):
            os.rename(f"{DOWNLOAD}/users.csv", f"{DOWNLOAD}/licenses.csv")
